...
.github/workflows/build.yml :: max=160
...
app/src/main/java/com/newoether/agora/api/DuckDuckGoScraper.kt :: max=24
...
app/src/test/java/com/newoether/agora/api/DuckDuckGoScraperTest.kt :: max=56
...
| 20 | `app/src/test/java/com/newoether/agora/api/DuckDuckGoScraperTest.kt` (raised 30 → 56) | F11: the CAPTCHA / vqd / offset tests re-declared the production regexes inline, so they tested their own copies — a change to `DuckDuckGoScraper`'s constants left them green. The three constants are now `internal` and the tests use them, so the test measures the shipped pattern. **Session 4:** two tests for the input bounds the audit flagged — `maxResults` of 0 / −1 / `Int.MIN_VALUE` must be an empty success with no request, and a blank query must not be scraped |
| 21 | `app/src/main/java/com/newoether/agora/api/DuckDuckGoScraper.kt` (raised 8 → 24) | F11, same fix: `CAPTCHA_REGEX` / `VQD_REGEX` / `OFFSET_REGEX` go from `private val` to `internal val` so the tests can assert on the real pattern instead of a copy. Visibility only; no behaviour change. **Session 4:** a non-positive `maxResults` returned `Error(NO_RESULTS)` — "DuckDuckGo returned no results for this query" for a request that never fetched a page. `search` now returns an empty `Success` for it. Unreachable from the app (the tool clamps to 1..10), but it was a wrong answer from a public API |
...
| 28 | `.github/workflows/build.yml` (raised 100 → 140 → 160) | Session 4: the release job now **verifies** what it publishes — `apksigner verify --print-certs` on both APKs, a hard fail when the phone and watch certificates differ (the Data Layer refuses to pair them otherwise), and `SHA256SUMS` generated in CI and uploaded beside the APKs. Before this, both checks lived in a release-notes checklist a human ran after upload, and the audit confirmed no CI step produced the digest file. The two APK uploads also gain `if-no-files-found: error` and `retention-days: 14`. **Raised 140 → 160 (Session 5):** the guard's merge-base fix (`fetch-depth: 0` + a full-graph upstream fetch) plus the new `Gate 0 artifact checks` step that finally wires `audit_gate0.sh --ci` into CI — the only place the *pinned fork identity*, the phone's `lintVitalFdroidRelease` and the launcher-label checks ran. Headroom is deliberate: upstream edits this file often, and two syncs have already aborted on a one-line overshoot |
...
| 30 | `README.md` (max=400) | Session 5 audit (F5): the guard's root-`*.md` class match let this file — which **exists upstream** — be rewritten (392 changed lines: fork identity, `applicationId`, release channel, Wear module, update channel, the at-a-glance table) with no registration at all, while the same class of edit to a Kotlin file failed loudly. The class was narrowed to paths upstream does not own, and a modified file that exists upstream now requires registration regardless of which class it matches; this is that registration. Budget 400 = the 392 lines carried today plus a small margin for the marker and a future correction |
